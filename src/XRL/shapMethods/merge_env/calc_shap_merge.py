...
import random
import numpy as np
from bluesky_gym.wrappers.xrlMethods.state.saliency.merge_env_saliency import SaliencyMergeControl
from src.XRL.shapMethods.shap_explainers import get_background_data_merge, runMergeExplainer
...
if __name__ == "__main__":
    ...
    parser.add_argument('--bg_samples', type=int, default=50)
    ...
    print(f"Phase 1: Collecting {args.bg_samples} deterministic background trajectories...")
    bg_env = gym.make(env_name)
    bg_env.action_space.seed(args.seed)
    bg_env.observation_space.seed(args.seed)
    bg_data = get_background_data_merge(bg_env, model, seed=args.seed, n_samples=args.bg_samples)
    bg_env.close()
    ...
    saliencyEnv = SaliencyMergeControl(env, fps=5, model=model, export_gifs_path=args.export_path)
    ...
    for i in range(args.n_eps):
        ...
        while not (done or truncated) and step < args.max_steps:
            ...
            # Multi-Output Exact SHAP (8 Coalitions)
            shap_values = runMergeExplainer(model, obs, bg_data, n_samples=args.bg_samples)
