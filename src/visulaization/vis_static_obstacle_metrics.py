...
def load_csv(file_path, metrics, align='right'):
...
def load_folder_data(folder_path, metrics, align='right'):
    all_files = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if f.endswith('.csv')]
    df_list = [load_csv(f, metrics, align=align) for f in all_files]
    if not df_list:
        return pd.DataFrame()
...
if __name__ == "__main__":
    ...
    metrics = ["crashed", "waypoint_reached"]
    ...
    if os.path.exists(logDir):
        ...
        combined_df = load_folder_data(logDir, metrics, align=args.align)
