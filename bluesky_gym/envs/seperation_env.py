...
WAYPOINT_DISTANCE_MIN = 1500
WAYPOINT_DISTANCE_MAX = 3000
...
class HorizontalCREnv(gym.Env):
    ...
    def _generate_conflicts(self, acid = 'KL001'):
        ...
        for i in range(NUM_INTRUDERS):
            self._spawn_intruder(str(i), target_idx)
    
    def _spawn_intruder(self, intruder_id, target_idx):
        ...
        bs.traf.creconfs(acid=intruder_id,actype="A320",targetidx=target_idx,dpsi=dpsi,dcpa=cpa,tlosh=tlosh)

    def _update_intruders(self):
        to_respawn = []
        for i in range(NUM_INTRUDERS):
            intruder_id = str(i)
            intruder_idx = bs.traf.id2idx(intruder_id)
            if intruder_idx < 0:
                to_respawn.append(intruder_id)
                continue
            
            ac_idx = bs.traf.id2idx('KL001')
            _, int_dis_nm = bs.tools.geo.kwikqdrdist(bs.traf.lat[ac_idx], bs.traf.lon[ac_idx], bs.traf.lat[intruder_idx], bs.traf.lon[intruder_idx])
            
            if int_dis_nm * NM2KM > DETECTION_RANGE:
                to_respawn.append(intruder_id)
        
        target_idx = bs.traf.id2idx('KL001')
        for intruder_id in to_respawn:
            idx = bs.traf.id2idx(intruder_id)
            if idx >= 0:
                bs.traf.delete(idx)
                # re-fetch target index because deleting might shift indices
                target_idx = bs.traf.id2idx('KL001')
            
            self._spawn_intruder(intruder_id, target_idx)
    ...
    def _get_obs(self):
        ...
        observation = {
                "intruder_distance": np.array(self.intruder_distance)/DETECTION_RANGE,
    ...
    def _get_reward(self):
        ...
        total_reward = reach_reward + drift_reward + intrusion_reward + ALIVE_REWARD
        self.total_reward += total_reward

        if self.total_intrusions > 0:
