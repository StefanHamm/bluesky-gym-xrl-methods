...
class SaliencySectorControlV2(SaliencyMapV1Wrapper):
    ...
    def step(self, action, shap_values=None,examplePlane = None):
        ...
        lat,lon = bs.traf.lat[bs.traf.id2idx(ACTOR)], bs.traf.lon[bs.traf.id2idx(ACTOR)]
        self.ownshiplatlon.append((lat,lon))
        for i,val in enumerate(self.ownshiplatlon):
            if i>len(self.path_coordinates)-1:
                break
            print(f"{self.path_coordinates[i-1]}: Lat {val[0]}, Lon {val[1]} {i}")
                ...
                if int_idx in obs_rank_map:
                    ...
                    if rank < len(shap_values.values[0]):
                        ...
                        if self.color_mode == self.color_map["quantitized"]:
                            ...
                            bar_x = (self.unwrapped.window_width/2)+(np.sin(np.deg2rad(int_qdr))*(int_dis * NM2KM)*self.px_per_km)
                            bar_y = (self.unwrapped.window_height/2)-(np.cos(np.deg2rad(int_qdr))*(int_dis * NM2KM)*self.px_per_km)
